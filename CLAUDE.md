...
**HyperPanels links to these pages via a hand-curated map, not a reverse lookup.** `hyperpanels/search/app/data/hub-character-links.json` maps character name → hub slug; it's maintained manually and is not generated from this repo's `character/*.md` list. Not every HyperPanels character has a timeline/hub page, so a missing map entry just means no "Explore Timeline" link renders there — expected, not a bug. If this repo renames or removes a character page, that map can go stale; there's no automated check.
...
- **`/games/` is the games hub** (`games.md`, `permalink: /games/`), driven by `_data/games.yml` — a separate list from `_data/products.yml` (which is news-badge metadata only). One DOM per game: a `.game-icon` button and a `.game-panel` card. Desktop (>768px) hides the icons and lays the panels out as a card grid; mobile (<=768px) shows the icons as a springboard and opens a panel as a fixed-position modal (`.is-open`) over `.games-backdrop`, driven by the inline script at the bottom of `games.md`. Three gotchas the CSS already works around, all from `.prose` in `_layouts/page.html`: `.prose div a` underlines every anchor with `!important`, `.prose h2` forces a yellow color plus a blue left bar, and the `.game-panel` border shorthand outranks the generic `.border-*` utilities — hence the doubled-up `.prose .game-panel a` / `.game-panel .game-panel-title` / `.game-panel.border-*` selectors. Colors come from the red/yellow/blue `.card`/`.btn` variants only; no green/purple/orange variants exist. `icon_image` in `_data/games.yml` is the springboard icon, currently the wide card art cropped square by `object-fit: cover` — swap those paths when real square icon art lands. The tiles are styled as phone app icons: inset to `$springboard-tile-width` (82%) of their grid column, `border-radius: 22%`, a 1px colored border, and a gloss/bezel `.game-icon::after` overlay. That overlay hangs off the parent, not the tile, because an inset `box-shadow` on an `<img>` paints beneath the image content — so if the tile width or radius changes, change it in both places.
- **`/apps/` is the apps hub** (`apps.md`, `permalink: /apps/`) for the two non-game apps — Timeline and HyperPanels — driven by `_data/apps.yml`. It's a straight copy of `/games/`'s markup and inline script (same `.game-icon`/`.game-panel`/`.games-springboard`/`.games-panels`/`.games-backdrop` classes from `_sass/_new-design.scss`, reused as-is rather than duplicated under an `app-*` name), just pointed at a different data file and different `data-launch-surface` values (`apps-hub-button`, `apps-hub-card`). Keep the two lists (`_data/apps.yml`, `_data/games.yml`) separate — they mirror the nav split between the Apps and Games dropdowns — and if `/games/`'s markup or script changes, port the change here too since nothing shares it structurally. Each hub ends its springboard with a hand-written `<a class="game-icon">` tile pointing at the other hub (Apps ↔ Games, a 3×3 grid and a gamepad, in a `.game-icon-glyph` box since neither has card art). Those two glyphs are **inline SVG, not FontAwesome** — the kit at `kit.fontawesome.com/13cabfa89d.js` did not render `fa-grip`, so don't reach for an `<i class="fas …">` here without checking it resolves first. Springboard art tiles need `.prose`-qualified rules to beat `.prose img`, which otherwise adds a 1rem block margin (misaligning the SVG tile, which is a `<span>`) and a white border shorthand (killing the product colors). The springboard is `display: none` above 768px, so those tiles are mobile-only by construction; the panel-opening listener selects `.game-icon[aria-controls]` so it skips them and the anchor just navigates.
- **Mobile menu is a grouped panel, not a flat list.** `_includes/header.html`'s `#mobile-nav` renders one `.mobile-nav-item` row per destination (icon tile + uppercase label), with `Apps` and `Games` wrapped in a `.mobile-nav-group` that carries a `.mobile-nav-grouphead` label, hairline rule, and an "All Apps"/"All Games" link to the parent's own `url`. `icon` (FontAwesome class) and `color` (hex) in `_data/navigation.yml` feed the row's icon and its `--nav-accent` custom property — colors match `_data/products.yml`, and the desktop nav ignores both keys. Active row is `page.url == url`, plus a `contains` check on top-level items so a character page lights up Characters. The panel offsets below the header via `--header-h`, set from `header.offsetHeight` by the script at the bottom of `header.html` on open and on resize; `.site-brand` gets `z-index: 45` so the wordmark stays above the panel's `z-index: 40`. Note the panel needs that literal `z-index` — `.z-40` is not a class that exists in `_sass/`, so the old markup's `z-40` did nothing.
- **Nav parents with `children` render as a `<button>` (toggle-only) unless the item also sets `url`, in which case the title renders as a link** (`_includes/header.html:27-34` desktop, `:46-52` mobile) — that's how both Apps and Games are dropdowns (of `/star-wars-timeline/`+`/hyper-panels/`, and of the three game landing pages) that are also direct links, to `/apps/` and `/games/` respectively.
...
- **Launch tracking:** any element with `data-launch-app="<product key>"` (plus optional `data-launch-surface`) fires one GA4 `launch_app` event. The delegated listener lives at the bottom of `_includes/footer.html` and runs in the **bubble** phase on purpose — nested READ links call `event.stopPropagation()`, so they never reach it and never mis-attribute as a launch. That same `stopPropagation` is why `games.md` binds its own direct listeners instead of relying on the delegated one. Tagged surfaces: `home-card`, `landing`, `footer-band`, `games-hub-button`, `games-hub-card`, `apps-hub-button`, `apps-hub-card`.
...
- **Header wordmark:** `_includes/header.html` renders `.site-brand` — "Aurebesh / Files" in yellow (`A New Hope` webfont, right-aligned) beside the same words in Aurebesh (`AurebeshAF` webfont, `aria-hidden`), matching `assets/social.png`. Replaced the old `<pre>` ASCII art. Both `@font-face` blocks and the styles live in `_sass/_custom.scss`; files are `assets/fonts/ANewHope.woff2` and `assets/fonts/AurebeshAF-Canon.woff2` (the Aurebesh one is the same face SWordle and Clone Defense ship — copied from `clone-defense/fonts/`, not from this repo's synced `clone-defense/play/` copy). AurebeshAF maps upper and lower case to the same glyphs, so no `text-transform` is needed. Both faces are `font-display: block` (not `swap`) and are `rel=preload`ed in `_includes/head.html`, with a one-year immutable `Cache-Control` for `/assets/fonts/*` in `netlify.toml` — without all three the wordmark flashed a fallback on every navigation, and the Aurebesh face has no sane fallback (it renders plain latin letters). The filenames aren't content-hashed, so replace a face by renaming the file and updating the `@font-face`, the preload, and nothing else.
