...
- `flappy-x-wing` is the newest app (added 2026-08-11) and is **still in development**. **The local folder is `flappy-x-wing` but the game is named Red Five**, and the public URL uses the game name, not the folder name. Hub-side state as of 2026-08-11:
  - **Done:** `red-five.md` landing page (`permalink: /red-five/`, `app:` block → `https://starwars.guide/red-five/play/`), `assets/cards/red-five.png`, `assets/red-five/icons/` sprites, a `red-five` row in `_data/products.yml` (**yellow-400** — it was briefly orange-400; the leftover `--orange-400` variable and `.text-orange-400` class in `_sass/_new-design.scss` now serve only the Reddit social link and `.social-card.border-orange-400`), and a `red-five/play` `sitemap: false` default in `_config.yml`.
