...
- **`_data/news.json`** — short items. Fields: `id` (kebab slug, used as the `/news/#anchor`), `date` (ISO), `product` (key from `_data/products.yml`), `title`, `message` (HTML allowed), optional `url` for a "Read more" link. **`url` must be an absolute `https://starwars.guide/...` URL** — the satellite apps render these items on their own origins, so a root-relative path would resolve against wordle./timeline./hyperpanels.starwars.guide and 404.
- **`_posts/*.md`** — long-form. Each post needs a `product:` front-matter key (defaults to `site`). **Don't use `app:` for this** — that key already triggers `WebApplication` JSON-LD in `structured-data.html`.
...
- Satellite apps should fetch `/news-feed.json` instead of shipping their own list. SWordle still reads its bundled `src/news.json` — migrating it is the outstanding follow-up.
